...
<template>
  <div class="group/list relative my-1">
    <div
      class="flex items-center group/item no-underline text-sm uppercase font-medium py-3 px-2 rounded-md cursor-pointer transition-all duration-300"
      :class="{
    ...
    <div
      v-if="children?.length > 0"
      class="absolute top-full right-0 pt-3 z-40 invisible transition-all duration-150 opacity-0 group-hover/list:opacity-100 group-hover/list:visible"
    >
